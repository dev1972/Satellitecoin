...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("STLLMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("STLLMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("STLLMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("stll-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running STLLMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
